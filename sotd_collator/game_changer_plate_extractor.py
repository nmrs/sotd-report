...
class GameChangerPlateExtractor(BaseNameExtractor):
    ...
    def get_name(self, comment):
        ...
        try:
            plate = self.plate_re.search(extracted_name).group(1).replace('.','').replace(',','')
        except AttributeError:
            return None

        if len(plate) == 2: plate = f'.{plate}' 
        elif len(plate) == 3: plate = f'{plate[0]}.{plate[1:3:1]}'

        plate = '{0}-P'.format(str(round(float(plate.removeprefix('0')), 2)).removeprefix('0'))
